...
class GetPostPDF : AddReactionDelegate,SharePostDelegate,comment_CountsDelegate{
    ...
    @IBAction func NormalTapped(gesture: UIGestureRecognizer){
        ...
        case .online(.wwan), .online(.wiFi):
            let cell = self.tableView.cellForRow(at: IndexPath(row: 0, section: gesture.view!.tag + 3)) as! PostPDFCell
    ...
    func addReaction(reation: String) {
        let cell = self.tableView.cellForRow(at: IndexPath(row: 0, section: self.selectedIndex + 3)) as! PostPDFCell
        self.audioPlayer.play()
