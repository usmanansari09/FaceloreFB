...
class GetPostMp3 :AddReactionDelegate,SharePostDelegate,comment_CountsDelegate{
    ...
    @IBAction func NormalTapped(gesture: UIGestureRecognizer){
        ...
        case .online(.wwan), .online(.wiFi):
            let cell = self.tableView.cellForRow(at: IndexPath(row: 0, section: gesture.view!.tag + 3)) as! MusicCell
    ...
    func addReaction(reation: String) {
        let cell = self.tableView.cellForRow(at: IndexPath(row: 0, section: self.selectedIndex + 3)) as! MusicCell
        self.audioPlayer.play()
...
//                            BoostPostManager.sharedInstance.boostPosts(targetController: self.targetController, postId: post_id ?? "")
    ...
    @IBAction func PlayBtn (sender:UIButton){
        let cell = self.tableView.cellForRow(at: IndexPath(row: 0, section: sender.tag + 3)) as! MusicCell
    ...
    func stopSound (){
     ...
     @IBAction func updateSlider(){
         print("update")
        let cell = self.tableView.cellForRow(at: IndexPath(row: 0 , section: self.selectIndex! + 3)) as! MusicCell
