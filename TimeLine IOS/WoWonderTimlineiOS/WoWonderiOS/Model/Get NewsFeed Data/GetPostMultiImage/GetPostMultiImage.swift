...
//                }
            ...
            cell.addREact = {[unowned self] in
                print(self.selectedIndex)
                  let cell = self.tableView.cellForRow(at: IndexPath(row: 0, section: self.selectedIndex + 3)) as! MultiImage2
                  cell.addREact?()
                ...
                case .online(.wwan), .online(.wiFi):
                    if let images_count = index["photo_multi"] as? [[String:Any]]{
                        if images_count.count == 2{
                            let cell = self.tableView.cellForRow(at: IndexPath(row: 0, section: indexpath.row + 3)) as! MultiImage2
                            if let reactions = index["reaction"] as? [String:Any]{
                                var totalCount = 0
                                if let count = reactions["count"] as? Int{
                                    totalCount = count
    ...
    @IBAction func NormalTapped(gesture: UIGestureRecognizer){
        ...
        case .online(.wwan), .online(.wiFi):
            ...
            if let images_count = self.postArray[gesture.view!.tag]["photo_multi"] as? [[String:Any]]{
                if images_count.count == 2{
                    let cell = self.tableView.cellForRow(at: IndexPath(row: 0, section: gesture.view!.tag + 3)) as! MultiImage2
                ...
                cell.LikeBtn.setTitle("\(" ")\(NSLocalizedString("Like", comment: "Like"))", for: .normal)
                    ...
                    let cell = self.tableView.cellForRow(at: IndexPath(row: 0, section: gesture.view!.tag + 3)) as! MultiImage3
                    if let reactions = self.postArray[gesture.view!.tag]["reaction"] as? [String:Any]{
                        var totalCount = 0
                        if let count = reactions["count"] as? Int{
                            totalCount = count
        ...
        self.audioPlayer.play()
           if self.imageCount == 2{
            ...
            self.imageCount = 3
                ...
                let cell = self.tableView.cellForRow(at: IndexPath(row: 0, section: self.selectedIndex + 3)) as! MultiImage3
